...
void GroundSegmentation::segment(const PointCloud &cloud,
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr res(
        new pcl::PointCloud<pcl::PointXYZI>());
    for (size_t i = 0; i < cloud.size(); ++i) {
        pcl::PointXYZI tt(cloud[i].x, cloud[i].y, cloud[i].z);
        if (segmentation->at(i) == 1)
            tt.intensity = 255;
        else tt.intensity = 0;
        // tt.intensity = segmentation->at(i);
        res->emplace_back(tt);
    }
    pcl::io::savePCDFileBinary(
        "/home/xavier/repos/linefit_ground_segmentation/match/res.pcd", *res);
