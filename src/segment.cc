...
Segment::Segment(const unsigned int& n_bins, const double& min_slope,
...
void Segment::fitSegmentLines() {
    ...
    std::list<Bin::MinZPoint> current_line_points(1,
                                                  line_start->getMinZPoint());
    LocalLine cur_line = std::make_pair(0, 0);
    for (auto line_iter = line_start + 1; line_iter != bins_.end();
         ++line_iter) {
        if (line_iter->hasPoint()) {
            ...
            if (current_line_points.size() >= 2) {
                ...
                if (error > max_error_ ||
                    ...
                    (current_line_points.size() > 2 &&
                     std::fabs(cur_line.first) < min_slope_) ||
                    is_long_line && std::fabs(expected_z - cur_point.z) >
                                        max_long_height_) {
                    ...
                    if (current_line_points.size() >= 3) {
                        const LocalLine new_line =
                            fitLocalLine(current_line_points);
                        lines_.push_back(
                            localLineToLine(new_line, current_line_points));
                        cur_ground_height =
                            new_line.first * current_line_points.back().d +
                            new_line.second;
                    }
                    // Start new line.
                    is_long_line = false;
                ...
                // Not enough points, try to start a new line
                // enough close and low than start height
                // (Xavier: Very important to set correctly max_start_height_
                // and sensor_height)
                if (cur_point.d - current_line_points.back().d <
                        long_threshold_ &&
                    std::fabs(current_line_points.back().z -
                              cur_ground_height) < max_start_height_) {
                    // Add point if valid.
                    current_line_points.push_back(cur_point);
                } else {
                    // Start new line.
...
double Segment::verticalDistanceToLine(const double& d, const double& z) {
    ...
    for (auto it = lines_.begin(); it != lines_.end(); ++it) {
        if (it->first.d - kMargin < d && it->second.d + kMargin > d) {
            ...
            distance_list.emplace_back(std::fabs(z - expected_z));
        }
    }
    std::sort(distance_list.begin(), distance_list.end(), std ::less<double>());
    return distance_list.size() ? distance_list[0] : -1.f;
}

double Segment::getMeanError(const std::list<Bin::MinZPoint>& points,
...
double Segment::getMaxError(const std::list<Bin::MinZPoint>& points,
                            const LocalLine& line) {
    double max_error = 0;
    for (auto it = points.begin(); it != points.end(); ++it) {
        const double residual = (line.first * it->d + line.second) - it->z;
