...
Viewer::Viewer() {
    ...
    viewer_.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0.0f, 1.0f, 0.0f,
        "min_cloud");
    viewer_.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2.0f, "min_cloud");
    viewer_.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 1.0f, 0.0f, 0.0f,
        "ground_cloud");
...
void Viewer::visualizeLines(const std::list<PointLine>& lines) {
    ...
    for (auto it = lines.begin(); it != lines.end(); ++it) {
        viewer_.addLine<PointType>(it->first, it->second,
